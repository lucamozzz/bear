import {
  ...
  NOT_REACHABLE_EVENT,
  DIFFERENT_POSITION_EVENT,
  DISCORDANT_MOVE_WHILE_BOUND_EVENT,
  NO_ROOT_EVENT
...
export default function SimulationState(
  ...
  eventBus.on(GUARD_VIOLATION_EVENT, context => {
    ...
    elementNotifications.addElementNotification(element, {
      ...
      text: 'Violated guard!',
      scope: {colors: {primary: '#FFDE21', auxiliary: '#000'}}
    });
  });
  ...
  eventBus.on(DIFFERENT_POSITION_EVENT, context => {
    const { element } = context;
    console.log('Handling DIFFERENT_POSITION_EVENT:', context);

    elementNotifications.addElementNotification(element, {
      type: 'warning',
      icon: ExclamationTriangleIcon(),
      text: 'The participants are in different positions!',
      scope: {colors: {primary: '#FFDE21', auxiliary: '#000'}}
    });
  });
  ...
  eventBus.on(DISCORDANT_MOVE_WHILE_BOUND_EVENT, context => {
    const { element } = context;
    console.log('Handling MOVE_WHILE_BOUND_EVENT:', context);

    elementNotifications.addElementNotification(element, {
      type: 'warning',
      icon: ExclamationTriangleIcon(),
      text: 'Discordant movement while bound!',
    });
  });
