...
import { NOT_REACHABLE_EVENT, NO_ROOT_EVENT } from "../../util/EventHelper";
...
ActivityBehavior.prototype.enter = async function (context) {
  ...
  if (element.businessObject.name.startsWith('Meet') ||
    element.businessObject.name.startsWith('Accompany') ||
    element.businessObject.name.startsWith('Stay') ||
    element.businessObject.name.startsWith('Get into') ||
    element.businessObject.name.startsWith('Load') ||
    element.businessObject.name.startsWith('Go with')) {
    let bind_ids = []
  ...
  if (element.businessObject.name.startsWith('Leave') ||
    element.businessObject.name.startsWith('Thank') ||
    element.businessObject.name.startsWith('Arrive') ||
    element.businessObject.name.startsWith('Join')) {
    let bind_ids = []
    let bind_flow = element.incoming.find(f => f.type === 'bpmn:MessageFlow');
    if (!bind_flow)
      bind_flow = element.outgoing.find(f => f.type === 'bpmn:MessageFlow');
