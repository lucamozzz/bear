...
ActivityBehavior.prototype._handleBoundaryEvents = function (context) {
  ...
  element.attachers.forEach(attacher => {
    if ((attacher.businessObject.eventDefinitions[0].condition.body == 'UNREACHABLE')) {
      ...
      return true;
    } else if (this._evaluateCondition(attacher, attacher.businessObject.eventDefinitions[0].condition.body)) {
      this._simulator.enter({
        element: attacher.outgoing[0],
        scope: scope.parent
      });
...
ActivityBehavior.prototype.processAssignments = function (context) {
  ...
  if (element.type === 'bpmn:Task' && element.businessObject.assignment) {
    ...
    if (assignmentObj[parentId] && assignmentObj[parentId]['enable']) {
      assignmentObj[parentId]['enable'].split(',').forEach(placeName => {
        this.addPlacesAndTransition(placeName.split('.'));
      });
    }

    if (assignmentObj[parentId] && assignmentObj[parentId]['disable']) {
      assignmentObj[parentId]['disable'].split(',').forEach(placeName => {
...
ActivityBehavior.prototype._processDestination = async function (context, element, activatedFlows, parentScope, scope) {
  ...
  if (flows.length === 0) {
    ...
    this._simulator.exit(context);
  } else if (conditionalEvent) {
    let condition = this._evaluateCondition(conditionalEvent, conditionalEvent.businessObject.eventDefinitions[0].condition.body);
    if (!condition) {
...
ActivityBehavior.prototype._handleDestination = async function (context, element, activatedFlows, parentScope, scope) {
  if (element.businessObject.destination) {
    if (element.parent.businessObject.root) {
      await this._processDestination(context, element, activatedFlows, parentScope, scope)
    } else {
      const modeling = this._spaceModeler.get('modeling');
...
ActivityBehavior.prototype._handleConnections = function (flows, activatedFlows, parentScope) {
  ...
  if (connections.some(connection => connection === null)) {
    // const modeling = this._spaceModeler.get('modeling');
    // modeling.setColor([parentScope.element], {
    //   stroke: 'red',
    //   fill: '#ffa5a5'
    // });
    // this._eventBus.fire(NOT_REACHABLE_EVENT, { element: parentScope.element });
    // setTimeout(function () {
    //   modeling.setColor([parentScope.element], {
    //     stroke: 'black',
    //     fill: 'white'
