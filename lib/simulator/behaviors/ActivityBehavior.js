...
import { NOT_REACHABLE_EVENT, NO_ROOT_EVENT, DIFFERENT_POSITION_EVENT, GUARD_VIOLATION_EVENT, DISCORDANT_MOVE_WHILE_BOUND_EVENT } from "../../util/EventHelper";
...
ActivityBehavior.prototype.enter = async function (context) {
  ...
  if (element.businessObject.$attrs.type === 'bind') {
    ...
    bind_ids.push(bind_flow.businessObject.sourceRef.$parent.id.substring(8))
    bind_ids.push(bind_flow.businessObject.targetRef.$parent.id.substring(8))
...
ActivityBehavior.prototype._processGuard = function (element) {
  ...
  if (e.attributes.hasOwnProperty(attribute)) {
    if (operator === '!=') {
      ...
      console.log('e.attributes[attribute].toString()', e.attributes[attribute].toString(), 'value', value);
  ...
  await new Promise(resolve => {
    const listener = (event) => {
      ...
      if (cause === "destinationReached") {
        ...
        this._simulator.exit();
        resolve();
      } else if (cause === "discordantMovements") {
        this._eventBus.fire(DISCORDANT_MOVE_WHILE_BOUND_EVENT, { element: element });
        this._simulator.exit();
        resolve();
      }
    };
...
// ActivityBehavior.prototype._handleBind = async function (participant, element) {
//   document.dispatchEvent(new CustomEvent('bind_start',
//     {
//       detail: {
//         participant: participant
...
//   await new Promise(resolve => {
//     const listener = (event) => {
//       document.removeEventListener('bind_stop', listener);
//       resolve();
...
//     document.addEventListener('bind_stop', listener);
//   });
// }

// ActivityBehavior.prototype._handleUnbind = async function (participant, element) {
...
//   await new Promise(resolve => {
//     const listener = (event) => {
//       document.removeEventListener('unbind_stop', listener);
//       resolve();
//     };

//     document.addEventListener('unbind_stop', listener);
//   });
// }
