...
ProcessBehavior.prototype.signal = function (context) {
  ...
  if (startEvent.incoming.length == 0) {

    this._elementRegistry.getAll().forEach(element => {
      if (element.type === 'bpmn:Participant') {
        this._participantNames[element.businessObject.processRef.id] = element.businessObject.name 
      }
    })
    
    localStorage.setItem('participants', JSON.stringify(this._participantNames));
