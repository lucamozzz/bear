...
import ConditionalStartEventBehavior from "./behaviors/StartEventConditionBehavior";
...
export default function Simulator(injector, eventBus, elementRegistry) {
  ...
  setInterval(() => {
    const flows = []
    Object.entries(elementRegistry._elements).forEach(([key, value]) => {
      if (key.startsWith('Activity')) {
        let bind_flow = value.element.incoming.find(f => f.type === 'bpmn:MessageFlow');
        if (bind_flow)
          flows.push(bind_flow.id)
      }
    })
    document.dispatchEvent(new CustomEvent('bindFlows', { detail: { flows: flows } }));
  }, 500)
  ...
  function enter(context) {
    ...
    //metto in coda il token e se non è partito lo faccio partire
    //metto il token e l'elemento connection nel trace enter
    queue(scope, function () {
      ...
      //prendo il comportamento dell'elemento e richiamo l'enter dell'elemento
      //(nel caso del sequence flor richiama quello dell'activity behaviour)
      ...
      //posso settare qui il tempo?


      //cambio lo scope da mettere in coda
